...
def quick_look_tnr_lfr_only_E_final(filepathtnr,filepathlfr): # Only E and displaying white when no values are available
    ...
    for key in frequency_keys:
        for index_time,time in enumerate (times_lfr[key]):
            if times_lfr[key][index_time]-times_lfr[key][index_time-1]>datetime.timedelta(hours=1):
...
def quick_look_tnr_lfr_final_PE_PB_DOP(filepathtnr,filepathlfr): # Displaying only PE, PB and DOP and set a bottom limit on y axis (10.5 Hz)
    my_data_lfr=Data(filepath=filepathlfr)
    my_data_tnr=Data(filepath=filepathtnr)
    my_data_tnr._init_
