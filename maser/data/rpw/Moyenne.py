...
def moyenne(tnr_filepath,lfr_filepath,mode,margin=5,desired_time_start=datetime.datetime(2021,10,28,0,0,0,0),
    ...
    while np.abs(desired_time_start-times[0][index_tnr_start])>datetime.timedelta(seconds=margin):
        index_tnr_start=index_tnr_start+1

    index_tnr_end=index_tnr_start

    while np.abs(desired_time_end-times[0][index_tnr_end])>datetime.timedelta(seconds=margin):
    ...
    if mode==0:
        ...
        while np.abs(desired_time_start-times_N_F1[index_lfr_N_start])>datetime.timedelta(seconds=margin):
            index_lfr_N_start=index_lfr_N_start+1
        index_lfr_N_end=index_lfr_N_start
        while np.abs(desired_time_end-times_N_F1[index_lfr_N_end])>datetime.timedelta(seconds=margin):
    ...
    if mode==1:
        ...
        #print(times_B_F0)
        while np.abs(desired_time_start-times_B_F0[index_lfr_B_start])>datetime.timedelta(seconds=margin):
            index_lfr_B_start=index_lfr_B_start +1
        index_lfr_B_end=index_lfr_B_start
        while np.abs(desired_time_end-times_B_F0[index_lfr_B_end])>datetime.timedelta(seconds=margin):
            index_lfr_B_end=index_lfr_B_end+1
        #print (times_B_F0[index_lfr_B])
        #print (times_B_F1[index_lfr_B])
        #voltage_B=np.hstack((voltage_B_F1[index_lfr_B_start:index_lfr_B_end+1],voltage_B_F0[index_lfr_B_start:index_lfr_B_end+1]))
...
moyenne('/home/atokgozoglu/Documents/Maser/maser-data/maser/data/rpw/solo_L2_rpw-tnr-surv_20210701_V01.cdf','/home/atokgozoglu/Documents/formationPython/solo_L2_rpw-lfr-surv-bp1_20210701_V03.cdf'
            ,mode=0,margin=19,desired_time_start=datetime.datetime(2021,7,1,0,49,44,1),desired_time_end=datetime.datetime(2021,7,1,5,50,55,1))
